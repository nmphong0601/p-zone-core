...
class AdminCategory extends ShopCategory
{
    ...
    public static function getListTitleAdmin()
    {
        ...
        if (pz_config_global('cache_status') && pz_config_global('cache_category')) {
            if (!Cache::has($storeCache.'_cache_category_'.pz_get_locale())) {
                ...
                pz_set_cache($storeCache.'_cache_category_'.pz_get_locale(), self::$getListTitleAdmin);
            }
            return Cache::get($storeCache.'_cache_category_'.pz_get_locale());
        } else {
            if (self::$getListTitleAdmin === null) {
                self::$getListTitleAdmin = self::join($tableDescription, $tableDescription.'.category_id', $table.'.id')
                ->where('lang', pz_get_locale())
