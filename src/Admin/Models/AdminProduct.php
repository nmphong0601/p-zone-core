...
class AdminProduct extends ShopProduct
{
    ...
    {
        return pz_product_admin_select_list($dataFilter, $storeId);
    }
    ...
    {
        $tableProductStore = (new ShopProductStore)->getTable();
        $storeId = $storeId ? pz_clean($storeId) : session('adminStoreId');
        $type = $type ? pz_clean($type) : 'sku';
        $fieldValue = pz_clean($fieldValue);
        $pId = pz_clean($pId);
        $check =  $this
    ...
    public function renderAttributeDetailsAdmin($currency = null, $rate = null)
    {
        ...
        foreach ($details as $groupId => $detailsGroup) {
            ...
            foreach ($detailsGroup as $k => $detail) {
                ...
                $html .= '<label class="radio-inline"><input ' . (($k == 0) ? "checked" : "") . ' type="radio" name="add_att[' . $this->id . '][' . $groupId . ']" value="' . $valueOption . '">' . pz_render_option_price($valueOption, $currency, $rate) . '</label> ';
            }
        }
        return $html;
    }
