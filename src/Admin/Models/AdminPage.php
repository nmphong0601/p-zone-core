...
use Cache;
use PZone\Core\Front\Models\ShopPageDescription;
use PZone\Core\Front\Models\ShopPageStore;
...
class AdminPage extends ShopPage
{
    ...
    public static function getListTitleAdmin($storeId = null)
    {
        ...
        if (pz_config_global('cache_status') && pz_config_global('cache_page')) {
            if (!Cache::has($storeCache.'_cache_page_'.pz_get_locale())) {
                if (self::$getListTitleAdmin === null) {
                    $data = self::join($tableDescription, $tableDescription.'.page_id', $table.'.id')
                    ->where('lang', pz_get_locale());
                    if ($storeId) {
                        $tablePageStore = (new ShopPageStore)->getTable();
                ...
                pz_set_cache($storeCache.'_cache_page_'.pz_get_locale(), self::$getListTitleAdmin);
            }
            return Cache::get($storeCache.'_cache_page_'.pz_get_locale());
        } else {
            if (self::$getListTitleAdmin === null) {
                $data = self::join($tableDescription, $tableDescription.'.page_id', $table.'.id')
                ->where('lang', pz_get_locale());
