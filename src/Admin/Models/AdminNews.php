...
use PZone\Core\Front\Models\ShopNewsStore;
...
class AdminNews extends ShopNews
{
    ...
    public static function getListTitleAdmin($storeId = null)
    {
        ...
        if (pz_config_global('cache_status') && pz_config_global('cache_news')) {
            if (!Cache::has($storeCache.'_cache_news_'.pz_get_locale())) {
                ...
                pz_set_cache($storeCache.'_cache_news_'.pz_get_locale(), self::$getListTitleAdmin);
            }
            return Cache::get($storeCache.'_cache_news_'.pz_get_locale());
        } else {
            if (self::$getListTitleAdmin === null) {
                $data = self::join($tableDescription, $tableDescription.'.news_id', $table.'.id')
                ->where('lang', pz_get_locale());
