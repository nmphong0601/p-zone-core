...
class AdminTemplateOnlineController extends RootAdminController
{
    ...
    public function index()
    {
        ...
        $page = request('page') ?? 1;
        $url = config('p-zone.api_link').'/templates/?page[size]=20&page[number]='.$page;
        ...
        if (!empty($dataApi['data'])) {
            ...
            $resultItems = pz_language_render('product.admin.result_item', ['item_from' => $dataApi['from'] ?? 0, 'item_to' => $dataApi['to']??0, 'total' =>  $dataApi['total'] ?? 0]);
            $htmlPaging .= '<ul class="pagination pagination-sm no-margin pull-right">';
            if ($dataApi['current_page'] > 1) {
                $htmlPaging .= '<li class="page-item"><a class="page-link pjax-container" href="'.pz_route_admin('admin_template_online').'?page='.($dataApi['current_page'] - 1).'" rel="prev">«</a></li>';
            } else {
                for ($i = 1; $i < $dataApi['last_page']; $i++) {
                    if ($dataApi['current_page'] == $i) {
                        ...
                        $htmlPaging .= '<li class="page-item"><a class="page-link" href="'.pz_route_admin('admin_template_online').'?page='.$i.'">'.$i.'</a></li>';
                    }
                }
            }
            if ($dataApi['current_page'] < $dataApi['last_page']) {
                $htmlPaging .= '<li class="page-item"><a class="page-link pjax-container" href="'.pz_route_admin('admin_template_online').'?page='.($dataApi['current_page'] + 1).'" rel="next">»</a></li>';
            }
            $htmlPaging .= '</ul>';
        }
