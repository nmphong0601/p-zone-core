...
class AdminStoreInfoController extends RootAdminController
{
    ...
    */
        ...
        if (!$lang) {
            try {
                if ($name == 'type') {
                    ...
                    $error = 1;
                    $msg = pz_language_render('store.admin.value_cannot_change');
                } elseif ($name == 'domain') {
                    if (
                        $storeId == PZ_ID_ROOT 
                        || ((pz_check_multi_vendor_installed()) && pz_store_is_partner($storeId)) 
                        || pz_check_multi_store_installed()
                    ) {
                        // Only store root can edit domain
                        $domain = pz_process_domain_store($value);
                        if (AdminStore::where('domain', $domain)->where('id', '<>', $storeId)->first()) {
                            $error = 1;
                            $msg = pz_language_render('store.admin.domain_exist');
                        } else {
                    ...
                    if (AdminStore::where('code', $value)->where('id', '<>', $storeId)->first()) {
                        $error = 1;
                        $msg = pz_language_render('store.admin.code_exist');
                    } else {
                    ...
                    if (file_exists($fileProcess = resource_path() . '/views/templates/'.$value.'/Provider.php')) {
                        ...
                        if (function_exists('pz_template_install_store')) {
                            ...
                            $checkTemplateEnableStore = (new \PZone\Core\Front\Models\ShopStoreCss)
    ...
    public function index()
    {
        ...
        if (!$store) {
            $data = [
                'title' => pz_language_render('store.admin.title'),
                'subTitle' => '',
                'icon' => 'fas fa-cogs',
                'dataNotFound' => 1
            ];
