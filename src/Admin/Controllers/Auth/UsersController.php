<?php
namespace PZone\Core\Admin\Controllers\Auth;

use PZone\Core\Admin\Admin;
use PZone\Core\Admin\Models\AdminPermission;
use PZone\Core\Admin\Models\AdminRole;
use PZone\Core\Admin\Models\AdminUser;
...
class UsersController extends RootAdminController
{
    ...
    {
        $data = [
            'title'         => pz_language_render('admin.user.list'),
            'subTitle'      => '',
            'icon'          => 'fa fa-indent',
            'urlDeleteItem' => pz_route_admin('admin_user.delete'),
        ...
        $listTh = [
            'id'         => 'ID',
            'username'   => pz_language_render('admin.user.user_name'),
            'name'       => pz_language_render('admin.user.name'),
            'roles'      => pz_language_render('admin.user.roles'),
            'permission' => pz_language_render('admin.user.permission'),
        ...
        $arrSort = [
            ...
            'username__desc' => pz_language_render('filter_sort.alpha_desc', ['alpha' => 'username']),
            'alpha__asc'  => pz_language_render('filter_sort.alpha_asc', ['alpha' => 'username']),
        ...
        foreach ($dataTmp as $key => $row) {
            ...
            $dataTr[] = [
                ...
                'action' => '
                    <a href="' . pz_route_admin('admin_user.edit', ['id' => $row['id'] ? $row['id'] : 'not-found-id']) . '"><span title="' . pz_language_render('action.edit') . '" type="button" class="btn btn-flat btn-sm btn-primary"><i class="fa fa-edit"></i></span></a>&nbsp;
                    ' . ((Admin::user()->id == $row['id'] || in_array($row['id'], PZ_GUARD_ADMIN)) ? '' : '<span onclick="deleteItem(\'' . $row['id'] . '\');"  title="' . pz_language_render('action.delete') . '" class="btn btn-flat btn-sm btn-danger"><i class="fas fa-trash-alt"></i></span>')
                ,
            ];
        }
        ...
        //menuRight
        $data['menuRight'][] = '<a href="' . pz_route_admin('admin_user.create') . '" class="btn  btn-success  btn-flat" title="New" id="button_create_new">
        ...
        //menuSearch
        $data['topMenuRight'][] = '
                <form action="' . pz_route_admin('admin_user.index') . '" id="button_search">
                <div class="input-group input-group" style="width: 350px;">
                    <select class="form-control rounded-0 select2" name="sort_order" id="sort_order">
                    '.$optionSort.'
                    </select> &nbsp;
                    <input type="text" name="keyword" class="form-control rounded-0 float-right" placeholder="' . pz_language_render('admin.user.search_place') . '" value="' . $keyword . '">
    ...
    {
        $data = [
            'title'             => pz_language_render('admin.user.add_new_title'),
            'subTitle'          => '',
            'title_description' => pz_language_render('admin.user.add_new_des'),
            'icon'              => 'fa fa-plus',
            'user'              => [],
            'roles'             => $this->roles,
            'permissions'       => $this->permissions,
            'url_action'        => pz_route_admin('admin_user.create'),
        ];
    ...
    public function postCreate()
    {
        ...
        $validator = Validator::make($dataOrigin, [
            ...
            'username.regex' => pz_language_render('admin.user.username_validate'),
        ]);
        ...
        return redirect()->route('admin_user.index')->with('success', pz_language_render('action.create_success'));
    }
    ...
    public function edit($id)
    {
        ...
        $data = [
            ...
            'roles'             => $this->roles,
            'permissions'       => $this->permissions,
            'url_action'        => pz_route_admin('admin_user.edit', ['id' => $user['id']]),
    ...
    public function postEdit($id)
    {
        ...
        if (!in_array($user->id, PZ_GUARD_ADMIN)) {
            $roles = $data['roles'] ?? [];
            $permission = $data['permission'] ?? [];
            $user->roles()->detach();
...
//
        return redirect()->route('admin_user.index')->with('success', pz_language_render('action.edit_success'));
    }
    ...
    public function deleteList()
    {
        if (!request()->ajax()) {
            ...
            $ids = request('ids');
            $arrID = explode(',', $ids);
            $arrID = array_diff($arrID, PZ_GUARD_ADMIN);
