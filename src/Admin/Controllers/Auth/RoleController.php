...
class RoleController extends RootAdminController
{
    ...
    {
        $data = [
            'title' => pz_language_render('admin.role.list'),
            'subTitle' => '',
            'icon' => 'fa fa-indent',
            'urlDeleteItem' => pz_route_admin('admin_role.delete'),
        ...
        $listTh = [
            'id' => 'ID',
            'slug' => pz_language_render('admin.role.slug'),
            'name' => pz_language_render('admin.role.name'),
            'permission' => pz_language_render('admin.role.permission'),
            'created_at' => pz_language_render('admin.role.created_at'),
            'updated_at' => pz_language_render('admin.updated_at'),
        ...
        foreach ($dataTmp as $key => $row) {
            ...
            $dataTr[] = [
                ...
                'action' => ((in_array($row['id'], PZ_GUARD_ROLES)) ? '' : '
                    <a href="' . pz_route_admin('admin_role.edit', ['id' => $row['id'] ? $row['id'] : 'not-found-id']) . '"><span title="' . pz_language_render('action.edit') . '" type="button" class="btn btn-flat btn-sm btn-primary"><i class="fa fa-edit"></i></span></a>&nbsp;
                    ')
                    . ((in_array($row['id'], PZ_GUARD_ROLES)) ? '' : '<span onclick="deleteItem(\'' . $row['id'] . '\');"  title="' . pz_language_render('action.delete') . '" class="btn btn-flat btn-sm btn-danger"><i class="fas fa-trash-alt"></i></span>')
                ,
            ];
        }
        ...
        //menuRight
        $data['menuRight'][] = '<a href="' . pz_route_admin('admin_role.create') . '" class="btn  btn-success  btn-flat" title="New" id="button_create_new">
        ...
        //topMenuRight
        $data['topMenuRight'][] ='
                <form action="' . pz_route_admin('admin_role.index') . '" id="button_search">
    ...
    {
        $data = [
            'title' => pz_language_render('admin.role.add_new_title'),
            'subTitle' => '',
            'title_description' => pz_language_render('admin.role.add_new_des'),
            'icon' => 'fa fa-plus',
            'role' => [],
            'permission' => (new AdminPermission)->pluck('name', 'id')->all(),
            'userList' => (new AdminUser)->pluck('name', 'id')->all(),
            'url_action' => pz_route_admin('admin_role.create'),
    ...
    public function postCreate()
    {
        ...
        $validator = Validator::make($dataOrigin, [
            ...
            'slug.regex' => pz_language_render('admin.role.slug_validate'),
        ]);
        ...
        return redirect()->route('admin_role.index')->with('success', pz_language_render('action.create_success'));
    }
    ...
    public function edit($id)
    {
        ...
        $data = [
            ...
            'permission' => (new AdminPermission)->pluck('name', 'id')->all(),
            'userList' => (new AdminUser)->pluck('name', 'id')->all(),
            'url_action' => pz_route_admin('admin_role.edit', ['id' => $role['id']]),
        ];
        return view($this->templatePathAdmin.'auth.role')
            ->with($data);
    }
    ...
    public function postEdit($id)
    {
        ...
        return redirect()->route('admin_role.index')->with('success', pz_language_render('action.edit_success'));
    }
    ...
    public function deleteList()
    {
        if (!request()->ajax()) {
            ...
            $ids = request('ids');
            $arrID = explode(',', $ids);
            $arrID = array_diff($arrID, PZ_GUARD_ROLES);
