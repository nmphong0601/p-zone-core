...
class AdminPluginsOnlineController extends RootAdminController
{
    ...
    protected function pluginCode($code)
    {
        ...
        $url = config('p-zone.api_link').'/plugins/'.$code.'?page[size]=20&page[number]='.$page;
        $url .='&version='.$pz_version;
        ...
        if (!empty($dataApi['data'])) {
            foreach ($dataApi['data'] as $key => $data) {
                $arrPluginLibrary[] = [
                    ...
                    'path' => $data['path'] ?? '',
                    'file' => $data['file'] ?? '',
                    'pzone_version' => $data['pzone_version'] ?? '',
            ...
            $resultItems = pz_language_render('admin.result_item', ['item_from' => $dataApi['from'] ?? 0, 'item_to' => $dataApi['to']??0, 'total' =>  $dataApi['total'] ?? 0]);
            $htmlPaging .= '<ul class="pagination pagination-sm no-margin pull-right">';
            if ($dataApi['current_page'] > 1) {
                $htmlPaging .= '<li class="page-item"><a class="page-link pjax-container" href="'.pz_route_admin('admin_plugin_online', ['code' => strtolower($code)]).'?page='.($dataApi['current_page'] - 1).'" rel="prev">«</a></li>';
            } else {
                for ($i = 1; $i < $dataApi['last_page']; $i++) {
                    if ($dataApi['current_page'] == $i) {
                        ...
                        $htmlPaging .= '<li class="page-item"><a class="page-link" href="'.pz_route_admin('admin_plugin_online', ['code' => strtolower($code)]).'?page='.$i.'">'.$i.'</a></li>';
                    }
                }
            }
            if ($dataApi['current_page'] < $dataApi['last_page']) {
                $htmlPaging .= '<li class="page-item"><a class="page-link pjax-container" href="'.pz_route_admin('admin_plugin_online', ['code' => strtolower($code)]).'?page='.($dataApi['current_page'] + 1).'" rel="next">»</a></li>';
            }
            $htmlPaging .= '</ul>';
        }
        $code = pz_word_format_class($code);

        $arrPluginLocal = pz_get_all_plugin($code);
    ...
    public function install()
    {
        ...
        try {
            ...
            $unzip = pz_unzip(storage_path('tmp/'.$pathTmp.'/'.$fileTmp), storage_path('tmp/'.$pathTmp));
            if ($unzip) {
                ...
                if (!pz_plugin_compatibility_check($pzoneVersion)) {
                    ...
                    $response = ['error' => 1, 'msg' => pz_language_render('admin.plugin.not_compatible', ['version' => $pzoneVersion, 'pz_version' => config('p-zone.core')])];
                } else {
