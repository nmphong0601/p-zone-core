...
use PZone\Core\Admin\Models\AdminTemplate;
...
class AdminTemplateController extends RootAdminController
{
    ...
    {
        $data = [
            'title' => pz_language_render('admin.template.list'),
            'subTitle' => '',
            'icon' => 'fa fa-indent',        ];

        $data["templates"] = pz_get_all_template();
    ...
    public function remove()
    {
        ...
        if (file_exists($fileProcess = resource_path() . '/views/templates/'.$key.'/Provider.php')) {
            ...
            if (function_exists('pz_template_uninstall')) {
                // Remove template from all stories
                pz_template_uninstall();
    ...
    public function refresh()
    {
        ...
        if (file_exists($fileProcess = resource_path() . '/views/templates/'.$key.'/Provider.php')) {
            ...
            if (function_exists('pz_template_uninstall') && function_exists('pz_template_install')) {
                //Remove all stories
                pz_template_uninstall();
                //Install data default and data for root domain
                pz_template_install($data);
            }
        }
    ...
    public function disable()
    {
        ...
        $response = ['error' => 0, 'msg' => pz_language_render('action.disable_success')];
        return response()->json($response);
    }
    ...
    public function enable()
    {
        ...
        $response = ['error' => 0, 'msg' => pz_language_render('action.enable_success')];
        return response()->json($response);
    }
    ...
    {
        $data =  [
            'title' => pz_language_render('admin.template.import')
        ];
        ...
        if ($pathFile) {
            ...
            if ($unzip) {
                ...
                if ($checkConfig) {
                    ...
                    if (!$configKey) {
                        File::deleteDirectory(storage_path('tmp/'.$pathTmp));
                        return redirect()->back()->with('error', pz_language_render('admin.template.error_config'));
                    }

                    $arrTemplateLocal = pz_get_all_template();
                    if (array_key_exists($configKey, $arrTemplateLocal)) {
                        File::deleteDirectory(storage_path('tmp/'.$pathTmp));
                        return redirect()->back()->with('error', pz_language_render('admin.template.error_exist'));
                    }
                    try {
                        ...
                        if (file_exists($fileProcess = resource_path() . '/views/templates/'.$configKey.'/Provider.php')) {
                            ...
                            if (function_exists('pz_template_install')) {
                                //Install data default and data for root domain
                                pz_template_install($data);
                            }
                        }
                    ...
                    File::deleteDirectory(storage_path('tmp/'.$pathTmp));
                    return redirect()->back()->with('error', pz_language_render('admin.template.error_check_config'));
                }
            } else {
                return redirect()->back()->with('error', pz_language_render('admin.template.error_unzip'));
            }
        } else {
            return redirect()->back()->with('error', pz_language_render('admin.template.error_upload'));
        }
        
        if (count($config)) {
            (new AdminTemplate)->create(['key' => $config['configKey'] ?? '', 'name' => $config['name'] ?? '', 'status' => 1]);
        }
        return redirect()->route('admin_template.index')->with('success', pz_language_render('admin.template.import_success')); 
    }
}
