...
class AdminStoreConfigController extends RootAdminController
{
    ...
    public function index()
    {
        ...
        $data = [
            'title' => pz_language_render('admin.menu_titles.config_store_default'),
            'subTitle' => '',
            'icon' => 'fas fa-cogs',
        ];
        ...
        $taxs[0] = pz_language_render('admin.tax.non_tax');
        ...
        $data['captcha_page'] = [
            'register' => pz_language_render('admin.captcha.captcha_page_register'),
            'forgot'   => pz_language_render('admin.captcha.captcha_page_forgot_password'),
            'checkout' => pz_language_render('admin.captcha.captcha_page_checkout'),
            'contact'  => pz_language_render('admin.captcha.captcha_page_contact'),
            'review'   => pz_language_render('admin.captcha.captcha_page_review'),
        ];
        ...
        $data['pluginCaptchaInstalled']         = pz_get_plugin_captcha_installed();
        ...
        $data['storeId']                        = $id;
        $data['urlUpdateConfig']                = pz_route_admin('admin_config.update');
    ...
    */
    ...
    public function addNew() {
        ...
        if (session('adminStoreId') != PZ_ID_ROOT && $storeId != session('adminStoreId')) {
            return response()->json(['error' => 1, 'msg' => pz_language_render('admin.remove_dont_permisison') . ': storeId#' . $storeId]);
        }

        if (!$key) {
            return redirect()->back()->with('error', 'Key: '.pz_language_render('admin.not_empty'));
        }
        ...
        if (AdminConfig::where(['key' => $key, 'store_id' => $storeId])->first()) {
            return redirect()->back()->with('error', pz_language_quickly('admin.admin_custom_config.key_exist', 'Key already exist'));
        }
        $dataUpdate = pz_clean($dataUpdate, [], true);
        AdminConfig::insert($dataUpdate);
        return redirect()->back()->with('success', pz_language_render('action.update_success'));
    }
