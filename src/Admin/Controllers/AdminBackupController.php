...
class AdminBackupController extends RootAdminController
{
    ...
    {
        $tableInfo = DB::connection(PZ_CONNECTION)
            ->table('INFORMATION_SCHEMA.TABLES')
            ->select(['TABLE_NAME', 'TABLE_ROWS', 'DATA_LENGTH'])
            ->where('TABLE_SCHEMA', config('database.connections.'.PZ_CONNECTION.'.database'))
        ...
        return view($this->templatePathAdmin.'screen.backup')->with(
            [
                "title"    => pz_language_render('admin.backup.title'),
                "arrFiles" => $arrFiles,
                "tableInfo" => $tableInfo,
            ]
        )->render();
    ...
    public function processBackupFile()
    {
        ...
        if ($action === 'remove') {
            try {
                unlink($pathFull);
                $return = ['error' => 0, 'msg' => pz_language_render('action.remove_success')];
            } catch (\Throwable $e) {
                $return = ['error' => 1, 'msg' => $e->getMessage()];
            }
        } elseif ($action === 'restore') {
            try {
                $return = Artisan::call("pz:restore --path=".$file);
            } catch (\Throwable $e) {
    ...
    public function generateBackup()
    {
        ...
        $return = Artisan::call("pz:backup --path=$fileName --includeTables=$includeTables --excludeTables=$excludeTables");
        return $return;
    }
}
