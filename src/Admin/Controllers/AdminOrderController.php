...
use PZone\Core\Front\Models\ShopPaymentStatus;
use PZone\Core\Front\Models\ShopShippingStatus;
use PZone\Core\Admin\Models\AdminCustomer;
use PZone\Core\Admin\Models\AdminOrder;
...
class AdminOrderController extends RootAdminController
{
    ...
    {
        $data = [
            'title'         => pz_language_render('order.admin.list'),
            'subTitle'      => '',
            'icon'          => 'fa fa-indent',
            'urlDeleteItem' => pz_route_admin('admin_order.delete'),
        ...
        $listTh = [
            'email'          => '<i class="fas fa-envelope" aria-hidden="true" title="'.pz_language_render('order.email').'"></i>',
            'subtotal'       => '<i class="fa fa-shopping-cart" aria-hidden="true" title="'.pz_language_render('order.subtotal').'"></i>',
            'shipping'       => '<i class="fa fa-truck" aria-hidden="true" title="'.pz_language_render('order.shipping').'"></i>',
            'discount'       => '<i class="fa fa-tags" aria-hidden="true" title="'.pz_language_render('order.discount').'"></i>',
            'tax'            => pz_language_render('order.tax'),
            'total'          => '<i class="fas fa-coins" aria-hidden="true" title="'.pz_language_render('order.total').'"></i>',
            'payment_method' => '<i class="fa fa-credit-card" aria-hidden="true" title="'.pz_language_render('order.admin.payment_method_short').'"></i>',
            'status'         => pz_language_render('order.admin.status'),
        ];
        if (pz_check_multi_shop_installed() && session('adminStoreId') == PZ_ID_ROOT) {
            // Only show store info if store is root
            $listTh['shop_store'] = '<i class="fab fa-shopify" aria-hidden="true" title="'.pz_language_render('front.store_list').'"></i>';
        }
        $listTh['created_at'] = pz_language_render('admin.created_at');
        ...
        $email        = pz_clean(request('email') ?? '');
        $from_to      = pz_clean(request('from_to') ?? '');
        $end_to       = pz_clean(request('end_to') ?? '');
        $order_status = pz_clean(request('order_status') ?? '');
        $arrSort = [
            ...
            'created_at__desc' => pz_language_render('filter_sort.value_desc', ['value' => 'Date']),
            'created_at__asc'  => pz_language_render('filter_sort.value_asc', ['value' => 'Date']),
        ];
        $dataSearch = [
            'keyword'      => $keyword,
            'email'        => $email,
        ...
        if (pz_check_multi_shop_installed() && session('adminStoreId') == PZ_ID_ROOT) {
            ...
            // Only show store info if store is root
            if (function_exists('pz_get_list_store_of_order')) {
                $dataStores = pz_get_list_store_of_order($arrId);
            } else {
                $dataStores = [];
            }
        }
        ...
        foreach ($dataTmp as $key => $row) {
            $dataMap = [
                'email'          => $row['email'] ?? 'N/A',
                'subtotal'       => pz_currency_render_symbol($row['subtotal'] ?? 0, $row['currency']),
                'shipping'       => pz_currency_render_symbol($row['shipping'] ?? 0, $row['currency']),
                'discount'       => pz_currency_render_symbol($row['discount'] ?? 0, $row['currency']),
                'tax'            => pz_currency_render_symbol($row['tax'] ?? 0, $row['currency']),
                'total'          => pz_currency_render_symbol($row['total'] ?? 0, $row['currency']),
            ...
            $dataMap['action'] = '<a href="' . pz_route_admin('admin_order.detail', ['id' => $row['id'] ? $row['id'] : 'not-found-id']) . '"><span title="' . pz_language_render('action.edit') . '" type="button" class="btn btn-flat btn-sm btn-primary"><i class="fa fa-edit"></i></span></a>&nbsp;
        ...
        //menuRight
        $data['menuRight'][] = '<a href="' . pz_route_admin('admin_order.create') . '" class="btn  btn-success  btn-flat" title="New" id="button_create_new">
        ...
        $data['topMenuRight'][] = '
                <form action="' . pz_route_admin('admin_order.index') . '" id="button_search">
                    <div class="input-group float-left">

                        <div style="width:130px">
                            <div class="form-group">
                                <label>'.pz_language_render('action.sort').':</label>
                        ...
                        <div style="width:130px">
                            <div class="form-group">
                                <label>'.pz_language_render('action.from').':</label>
                                <div class="input-group">
                        ...
                        <div style="width:130px">
                            <div class="form-group">
                                <label>'.pz_language_render('action.to').':</label>
                                <div class="input-group">
                        ...
                        <div style="width:150px">
                            <div class="form-group">
                                <label>'.pz_language_render('order.admin.status').':</label>
                                <div class="input-group">
                                <select class="form-control rounded-0" name="order_status">
                                <option value="">'.pz_language_render('order.admin.search_order_status').'</option>
                                ' . $optionStatus . '
                                </select>
                                </div>
                            </div>
                        ...
                        <div style="width:150px">
                            <div class="form-group">
                                <label>'.pz_language_render('order.admin.search_email').':</label>
                                <div class="input-group">
                                    <input type="text" name="email" class="form-control rounded-0 float-right" placeholder="' . pz_language_render('order.admin.search_email') . '" value="' . $email . '">
    ...
    {
        $data = [
            'title'             => pz_language_render('order.admin.add_new_title'),
            'subTitle'          => '',
            'title_description' => pz_language_render('order.admin.add_new_des'),
            'icon'              => 'fa fa-plus',
        ];
        $paymentMethod = [];
        $shippingMethod = [];
        $paymentMethodTmp = pz_get_plugin_installed('payment', $onlyActive = false);
        foreach ($paymentMethodTmp as $key => $value) {
            $paymentMethod[$key] = pz_language_render($value->detail);
        }
        $shippingMethodTmp = pz_get_plugin_installed('shipping', $onlyActive = false);
        foreach ($shippingMethodTmp as $key => $value) {
            $shippingMethod[$key] = pz_language_render($value->detail);
        }
    ...
    public function postCreate()
    {
        ...
        $messages = [
            ...
            'shipping_method.required' => pz_language_render('cart.validation.shippingMethod_required'),
            'payment_method.required'  => pz_language_render('cart.validation.paymentMethod_required'),
        ];
        ...
        AdminOrder::insertOrderTotal([
            ['id' => pz_uuid(),'code' => 'subtotal', 'value' => 0, 'title' => pz_language_render('order.totals.sub_total'), 'sort' => ShopOrderTotal::POSITION_SUBTOTAL, 'order_id' => $order->id],
            ['id' => pz_uuid(),'code' => 'tax', 'value' => 0, 'title' => pz_language_render('order.totals.tax'), 'sort' => ShopOrderTotal::POSITION_TAX, 'order_id' => $order->id],
            ['id' => pz_uuid(),'code' => 'shipping', 'value' => 0, 'title' => pz_language_render('order.totals.shipping'), 'sort' => ShopOrderTotal::POSITION_SHIPPING_METHOD, 'order_id' => $order->id],
            ['id' => pz_uuid(),'code' => 'discount', 'value' => 0, 'title' => pz_language_render('order.totals.discount'), 'sort' => ShopOrderTotal::POSITION_TOTAL_METHOD, 'order_id' => $order->id],
            ['id' => pz_uuid(),'code' => 'other_fee', 'value' => 0, 'title' => config('cart.process.other_fee.title'), 'sort' => ShopOrderTotal::POSITION_OTHER_FEE, 'order_id' => $order->id],
            ['id' => pz_uuid(),'code' => 'total', 'value' => 0, 'title' => pz_language_render('order.totals.total'), 'sort' => ShopOrderTotal::POSITION_TOTAL, 'order_id' => $order->id],
            ['id' => pz_uuid(),'code' => 'received', 'value' => 0, 'title' => pz_language_render('order.totals.received'), 'sort' => ShopOrderTotal::POSITION_RECEIVED, 'order_id' => $order->id],
        ]);
        //
        return redirect()->route('admin_order.index')->with('success', pz_language_render('action.create_success'));
    }
    ...
    public function detail($id)
    {
        ...
        $products = (new AdminProduct)->getProductSelectAdmin(['kind' => [PZ_PRODUCT_SINGLE, PZ_PRODUCT_BUILD]]);
        $paymentMethod = [];
        $shippingMethod = [];
        $paymentMethodTmp = pz_get_plugin_installed('payment', $onlyActive = false);
        foreach ($paymentMethodTmp as $key => $value) {
        ...
        return view($this->templatePathAdmin.'screen.order_edit')->with(
            [
                "title" => pz_language_render('order.order_detail'),
                "subTitle" => '',
    ...
    public function getInfoProduct()
    {
        ...
        if (!$product) {
            return response()->json(['error' => 1, 'msg' => pz_language_render('admin.data_not_found_detail', ['msg' => '#product:'.$id]), 'detail' => '']);
        }
    ...
    public function postOrderUpdate()
    {
        ...
        if ($code == 'shipping' || $code == 'discount' || $code == 'received' || $code == 'other_fee') {
            ...
            if (!$order) {
                return response()->json(['error' => 1, 'msg' => pz_language_render('admin.data_not_found_detail', ['msg' => 'order#'.$orderId]), 'detail' => '']);
            }
            $dataRowTotal = [
                'id' => $id,
                'code' => $code,
                'value' => $value,
                'text' => pz_currency_render_symbol($value, $order->currency),
            ];
            ...
            if ($code == 'status') {
                ...
                if ($oldValue !=  5 && $value == 5) {
                    if (function_exists('pz_order_success_finish')) {
                        pz_order_success_finish($orderId);
                    }
                }
                if ($oldValue ==  5 && $value != 5) {
                    if (function_exists('pz_order_success_unfinish')) {
                        pz_order_success_unfinish($orderId);
                    }
                }
                //Process finish order
            }
        ...
        $blance = '<tr ' . $style . ' class="data-balance"><td>' . pz_language_render('order.totals.balance') . ':</td><td align="right">' . pz_currency_format($orderUpdated->balance) . '</td></tr>';
        return response()->json(['error' => 0, 'detail' =>
            [
                'total' => pz_currency_format($orderUpdated->total),
                'subtotal' => pz_currency_format($orderUpdated->subtotal),
                'tax' => pz_currency_format($orderUpdated->tax),
                'shipping' => pz_currency_format($orderUpdated->shipping),
                'discount' => pz_currency_format($orderUpdated->discount),
                'other_fee' => pz_currency_format($orderUpdated->other_fee),
                'received' => pz_currency_format($orderUpdated->received),
                'balance' => $blance,
            ],
    ...
    public function postAddItem()
    {
        ...
        foreach ($addIds as $key => $id) {
            ...
            if ($id && $add_qty[$key]) {
                $product = AdminProduct::getProductAdmin($id);
                if (!$product) {
                    return response()->json(['error' => 1, 'msg' => pz_language_render('admin.data_not_found_detail', ['msg' => '#'.$id]), 'detail' => '']);
                }
    ...
    public function postEditItem()
    {
        try {
            ...
            $dataHistory = [
                'order_id' => $orderId,
                'content' => pz_language_render('product.edit_product') . ' #' . $id . ': ' . $field . ' from ' . $fieldOrg . ' -> ' . $value,
                'admin_id' => Admin::user()->id,
                'order_status_id' => $order->status,
            ];
            (new AdminOrder)->addOrderHistory($dataHistory);
            ...
            $arrayReturn = ['error' => 0, 'detail' => [
                ...
                'discount'         => pz_currency_format($orderUpdated->discount),
                'received'         => pz_currency_format($orderUpdated->received),
                'item_total_price' => pz_currency_render_symbol($item->total_price, $item->currency),
                'item_id'          => $id,
                'balance'          => $blance,
            ],'msg' => pz_language_render('action.update_success')
            ];
    ...
    public function postDeleteItem()
    {
        try {
            ...
            if (!$itemDetail) {
                return response()->json(['error' => 1, 'msg' => pz_language_render('admin.data_not_found_detail', ['msg' => 'detail#'.$pId]), 'detail' => '']);
            }
                ...
                foreach ($order->details as $key => $detail) {
                    ...
                    if ($arrAtt) {
                        ...
                        foreach ($arrAtt as $groupAtt => $att) {
                            $htmlAtt .= $attributesGroup[$groupAtt] .':'.pz_render_option_price($att, $order['currency'], $order['exchange_rate']);
                        }
