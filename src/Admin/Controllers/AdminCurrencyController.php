...
class AdminCurrencyController extends RootAdminController
{
    ...
    {
        $data = [
            'title' => pz_language_render('admin.currency.list'),
            'subTitle' => '',
            'icon' => 'fa fa-indent',
            'urlDeleteItem' => pz_route_admin('admin_currency.delete'),
        ...
        $listTh = [
            'name' => pz_language_render('admin.currency.name'),
            'code' => pz_language_render('admin.currency.code'),
            'symbol' => pz_language_render('admin.currency.symbol'),
            'exchange_rate' => pz_language_render('admin.currency.exchange_rate'),
            'precision' => pz_language_render('admin.currency.precision'),
            'symbol_first' => pz_language_render('admin.currency.symbol_first'),
            'thousands' => pz_language_render('admin.currency.thousands'),
            'sort' => pz_language_render('admin.currency.sort'),
            'status' => pz_language_render('admin.currency.status'),
        ...
        foreach ($dataTmp as $key => $row) {
            $dataTr[$row['id']] = [
                ...
                'action' => '
                    <a href="' . pz_route_admin('admin_currency.edit', ['id' => $row['id'] ? $row['id'] : 'not-found-id']) . '"><span title="' . pz_language_render('action.edit') . '" type="button" class="btn btn-flat btn-sm btn-primary"><i class="fa fa-edit"></i></span></a>&nbsp;

                  <span ' . (in_array($row['id'], PZ_GUARD_CURRENCY) ? "style='display:none'" : "") . ' onclick="deleteItem(\'' . $row['id'] . '\');"  title="' . pz_language_render('action.delete') . '" class="btn btn-flat btn-sm btn-danger"><i class="fas fa-trash-alt"></i></span>
                  ',
            ];
        }
        ...
        //menuRight
        $data['menuRight'][] = '<a href="' . pz_route_admin('admin_currency.create') . '" class="btn  btn-success  btn-flat" title="New" id="button_create_new">
        ...
        //menuSearch
        $data['topMenuRight'][] = '
                <form action="' . pz_route_admin('admin_currency.index') . '" id="button_search">
    ...
    {
        $data = [
            'title' => pz_language_render('admin.currency.add_new_title'),
            'subTitle' => '',
            'title_description' => pz_language_render('admin.currency.add_new_des'),
            'icon' => 'fa fa-plus',
            'currency' => [],
            'url_action' => pz_route_admin('admin_currency.create'),
        ];
    ...
    public function postCreate()
    {
        ...
        return redirect()->route('admin_currency.index')->with('success', pz_language_render('action.create_success'));
    }
    ...
    public function edit($id)
    {
        ...
        $data = [
            ...
            'icon' => 'fa fa-edit',
            'currency' => $currency,
            'url_action' => pz_route_admin('admin_currency.edit', ['id' => $currency['id']]),
        ];
        return view($this->templatePathAdmin.'screen.currency')
            ->with($data);
    }
    ...
    public function postEdit($id)
    {
        ...
        return redirect()->route('admin_currency.index')->with('success', pz_language_render('action.edit_success'));
    }
    ...
    public function deleteList()
    {
        if (!request()->ajax()) {
            ...
            $ids = request('ids');
            $arrID = explode(',', $ids);
            $arrID = array_diff($arrID, PZ_GUARD_CURRENCY);
