...
class AdminLengthController extends RootAdminController
{
    ...
    {
        $data = [
            'title' => pz_language_render('admin.length.list'),
            'title_action' => '<i class="fa fa-plus" aria-hidden="true"></i> ' . pz_language_render('admin.length.add_new_title'),
            'subTitle' => '',
            'icon' => 'fa fa-indent',
            'urlDeleteItem' => pz_route_admin('admin_length_unit.delete'),
            'removeList' => 0, // 1 - Enable function delete list item
            'buttonRefresh' => 0, // 1 - Enable button refresh
            'css' => '',
            'js' => '',
            'url_action' => pz_route_admin('admin_length_unit.create'),
        ];

        $listTh = [
            'id' => 'ID',
            'name' => pz_language_render('admin.length.name'),
            'description' => pz_language_render('admin.length.description'),
        ...
        foreach ($dataTmp as $key => $row) {
            $dataTr[$row['id']] = [
                ...
                'action' => '
                    <a href="' . pz_route_admin('admin_length_unit.edit', ['id' => $row['id'] ? $row['id'] : 'not-found-id']) . '"><span title="' . pz_language_render('action.edit') . '" type="button" class="btn btn-flat btn-sm btn-primary"><i class="fa fa-edit"></i></span></a>&nbsp;
    ...
    public function postCreate()
    {
        ...
        return redirect()->route('admin_length_unit.index')->with('success', pz_language_render('action.create_success'));
    }
    ...
    public function edit($id)
    {
        ...
        $data = [
            ...
            'css' => '',
            'js' => '',
            'url_action' => pz_route_admin('admin_length_unit.edit', ['id' => $length['id']]),
            'length' => $length,
            'id' => $id,
        ];
