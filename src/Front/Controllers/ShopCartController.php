...
class ShopCartController extends RootFrontController
{
    ...
    private function _getCart()
    {
        ...
        pz_check_view($this->templatePath . '.screen.shop_cart');
        return view(
            $this->templatePath . '.screen.shop_cart',
            [
                'title'           => pz_language_render('cart.cart_title'),
                'description'     => '',
                'keyword'         => '',
                'cart'            => $cart,
                ...
                'breadcrumbs'  => [
                    ['url' => '', 'title' => pz_language_render('cart.cart_title')],
                ],
            ]
        );
    }
    ...
    public function prepareCheckout()
    {
        ...
        //Not allow for guest
        if (!pz_config('shop_allow_guest') && !$customer) {
            return redirect(pz_route('login'));
        }
        ...
        if (!$storeId) {
            return redirect(pz_route('cart'))->with(['error' => pz_language_render('cart.cart_store_id_notfound')]);
        }
        ...
        if (empty($cartGroup[$storeId])) {
            return redirect(pz_route('cart'))->with(['error' => pz_language_render('cart.cart_store_empty')]);
        }
        ...
        if (count($arrErrorQty)) {
            return redirect(pz_route('cart'))->with(['arrErrorQty' => $arrErrorQty, 'error'=> pz_language_render('cart.have_error')]);
        }
        //End check minimum
        ...
        return redirect(pz_route('checkout'));
    }
    ...
    private function _getCheckout()
    {
        ...
        if (!$dataCheckout || !$storeCheckout) {
            return redirect(pz_route('cart'))->with(['error' => pz_language_render('cart.cart_empty')]);
        }

        //Shipping
        $moduleShipping = pz_get_plugin_installed('shipping');
        $sourcesShipping = pz_get_all_plugin('shipping');
        $shippingMethod = array();
        foreach ($moduleShipping as $module) {
            if (array_key_exists($module['key'], $sourcesShipping)) {
                $moduleClass = pz_get_class_plugin_config('shipping', $module['key']);
                $shippingMethod[$module['key']] = (new $moduleClass)->getData();
            }
        }

        //Payment
        $modulePayment = pz_get_plugin_installed('payment');
        $sourcesPayment = pz_get_all_plugin('payment');
        $paymentMethod = array();
        ...
        //Total
        $moduleTotal = pz_get_plugin_installed('total');
        $sourcesTotal = pz_get_all_plugin('total');
        $totalMethod = array();
        ...
        //Process captcha
        $viewCaptcha = '';
        if (pz_captcha_method() && in_array('checkout', pz_captcha_page())) {
            if (view()->exists(pz_captcha_method()->pathPlugin.'::render')) {
                $dataView = [
                    'titleButton' => pz_language_render('cart.checkout'),
        ...
        //Check view
        pz_check_view($this->templatePath . '.screen.shop_checkout');

        return view(
            $this->templatePath . '.screen.shop_checkout',
            [
                'title'           => pz_language_render('cart.checkout'),
                'description'     => '',
                'keyword'         => '',
                ...
                'breadcrumbs'     => [
                    ['url'        => '', 'title' => pz_language_render('cart.checkout')],
                ],
            ]
        );
    }
    ...
    public function processCheckout()
    {
        ...
        $dataMap = pz_order_mapping_validate();
        ...
        if (count($arrErrorQty)) {
            return redirect(pz_route('cart'))->with('arrErrorQty', $arrErrorQty);
        }
        //End check minimum

        return redirect(pz_route('checkout.confirm'))->with('step', 'checkout.confirm');
    }
    ...
    private function _getCheckoutConfirm()
    {
        ...
        if (
            ...
            return redirect(pz_route('cart'))->with(['error' => 'shippingAddress empty!']);
        }
        ...
        if (pz_config('shipping_off')) {
            ...
            if (!session('shippingMethod')) {
                return redirect(pz_route('cart'))->with(['error' => 'shippingMethod empty!']);
            }
            $shippingMethod = session('shippingMethod');
            $classShippingMethod = pz_get_class_plugin_config('Shipping', $shippingMethod);
        ...
        if (pz_config('payment_off')) {
            ...
            if (!session('paymentMethod')) {
                return redirect(pz_route('cart'))->with(['error' => 'paymentMethod empty!']);
            }
            $paymentMethod = session('paymentMethod');
            $classPaymentMethod = pz_get_class_plugin_config('Payment', $paymentMethod);
            $paymentMethodData = (new $classPaymentMethod)->getData();
        }

        //Check plugin invalid
        if (!pz_config(session('shippingMethod'))) {
            return redirect(pz_route('cart'))->with(['error' => 'Plugin shipping invalid!']);
        }
        if (!pz_config(session('paymentMethod'))) {
            return redirect(pz_route('cart'))->with(['error' => 'Plugin payment invalid!']);
        }
        ...
        pz_check_view($this->templatePath . '.screen.shop_checkout_confirm');
        return view(
            $this->templatePath . '.screen.shop_checkout_confirm',
            [
                'title'              => pz_language_render('checkout.page_title'),
                ...
                'breadcrumbs'        => [
                    ['url'           => '', 'title' => pz_language_render('checkout.page_title')],
                ],
            ]
        );
    }
    ...
    public function addOrder(Request $request)
    {
        ...
        $data = request()->all();
        if (!$data) {
            return redirect(pz_route('cart'))->with(['error' => 'Data empty!']);
        } else {
        ...
        $dataOrder['currency']        = pz_currency_code();
        $dataOrder['exchange_rate']   = pz_currency_rate();
        ...
        $dataOrder['created_at']      = pz_time_now();
        ...
        foreach ($dataCheckout as $cartItem) {
            ...
            $arrDetail['price']       = pz_currency_value($cartItem->price);
            ...
            $arrDetail['total_price'] = pz_currency_value($cartItem->price) * $cartItem->qty;
        ...
        if ($newOrder['error'] == 1) {
            pz_report($newOrder['msg']);
            return redirect(pz_route('cart'))->with(['error' => $newOrder['msg']]);
        }
        //Set session orderID
        ...
        if ($address_process == 'new') {
            ...
            //Process escape
            $addressNew = pz_clean($addressNew);
        ...
        $paymentMethod = pz_get_class_plugin_controller('Payment', session('paymentMethod'));
    ...
    public function addToCart()
    {
        ...
        if (!$product) {
            return response()->json(
                [
                    'error' => 1,
                    'msg' => pz_language_render('front.data_notfound'),
                ]
            );
        }
        ...
        if ($product->allowSale()) {
            ...
            Cart::add($dataCart);
            return redirect(pz_route('cart'))
                ->with(
                    ['success' => pz_language_render('cart.add_to_cart_success', ['instance' => 'cart'])]
                );
        } else {
            return redirect(pz_route('cart'))
                ->with(
                    ['error' => pz_language_render('product.dont_allow_sale', ['sku' => $product->sku])]
                );
        }
    }
    ...
    {
        if (!$request->ajax()) {
            return redirect(pz_route('cart'))->with(['error' => 'Method not allow!']);
        }
        $data     = request()->all();
        ...
        switch ($instance) {
            case 'default':
                if ($product->attributes->count() || $product->kind == PZ_PRODUCT_GROUP) {
                ...
                if ($product->allowSale()) {
                    ...
                    return response()->json(
                        [
                            'error' => 1,
                            'msg' => pz_language_render('product.dont_allow_sale', ['sku' => $product->sku]),
                        ]
                    );
                }
                break;
            ...
            default:
                ...
                if (!in_array($id, ${'arrID' . $instance})) {
                    ...
                    return response()->json(
                        [
                            'error' => 1,
                            'msg' => pz_language_render('cart.item_exist_in_cart', ['instance' => $instance]),
                        ]
                    );
                }
                break;
        ...
        return response()->json(
            [
                ...
                'msg'        => pz_language_render('cart.add_to_cart_success', ['instance' => ($instance == 'default') ? 'cart' : $instance]),
            ]
        );
    }
    ...
    public function updateToCart(Request $request)
    {
        ...
        if ($product->stock < $new_qty && !pz_config('product_buy_out_of_stock', $product->store_id)) {
            return response()->json(
                [
                    'error' => 1,
                    'msg' => pz_language_render('cart.item_over_qty', ['sku' => $product->sku, 'qty' => $new_qty]),
                ]
            );
        } else {
    ...
    private function _wishlist()
    {
        ...
        pz_check_view($this->templatePath . '.screen.shop_wishlist');
        return view(
            $this->templatePath . '.screen.shop_wishlist',
            array(
                'title'       => pz_language_render('cart.page_wishlist_title'),
                'description' => '',
                'keyword'     => '',
                'wishlist'    => $wishlist,
                'layout_page' => 'shop_wishlist',
                'breadcrumbs' => [
                    ['url'    => '', 'title' => pz_language_render('cart.page_wishlist_title')],
                ],
            )
        );
    }
    ...
    private function _compare()
    {
        ...
        pz_check_view($this->templatePath . '.screen.shop_compare');
        return view(
            $this->templatePath . '.screen.shop_compare',
            array(
                'title'       => pz_language_render('cart.page_compare_title'),
                'description' => '',
                'keyword'     => '',
                'compare'     => $compare,
                'layout_page' => 'shop_compare',
                'breadcrumbs' => [
                    ['url'    => '', 'title' => pz_language_render('cart.page_compare_title')],
                ],
            )
        );
    }
    ...
    private function _clearCart($instance = 'cart')
    {
        ...
        return redirect(pz_route($instance));
    }
    ...
    public function completeOrder()
    {
        ...
        $classPaymentConfig = pz_get_class_plugin_config('Payment', $paymentMethod);
        if (method_exists($classPaymentConfig, 'endApp')) {
            (new $classPaymentConfig)->endApp();
        }

        $classShippingConfig = pz_get_class_plugin_config('Shipping', $shippingMethod);
        ...
        if ($totalMethod && is_array($totalMethod)) {
            foreach ($totalMethod as $keyMethod => $valueMethod) {
                $classTotalConfig = pz_get_class_plugin_config('Total', $keyMethod);
        ...
        pz_event_order_success($order = ShopOrder::find($orderID));

        // Process after order compled: send mail, data response ...
        $dataResponse = $this->processAfterOrderSuccess($orderID);

        return redirect(pz_route('order.success'))->with($dataResponse);
    }
    ...
    private function _orderSuccess()
    {
        ...
        pz_check_view($this->templatePath . '.screen.shop_order_success');
        $orderInfo = ShopOrder::with('details')->find(session('orderID'))->toArray();
        return view(
            $this->templatePath . '.screen.shop_order_success',
            [
                'title'       => pz_language_render('checkout.success_title'),
                'orderInfo'   => $orderInfo,
                'layout_page' => 'shop_order_success',
                'breadcrumbs' => [
                    ['url'    => '', 'title' => pz_language_render('checkout.success_title')],
                ],
            ]
        );
    }
    ...
    private function processAfterOrderSuccess (string $orderID)
    {
        //Clear session
        $this->clearSession();
        return pz_order_process_after_success($orderID);
    }
}
