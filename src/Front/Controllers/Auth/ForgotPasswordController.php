...
class ForgotPasswordController extends RootFrontController
{
    ...
    public function sendResetLinkEmail(\Illuminate\Http\Request $request)
    {
        ...
        if (pz_captcha_method() && in_array('forgot', pz_captcha_page())) {
            $data['captcha_field'] = $data[pz_captcha_method()->getField()] ?? '';
            $dataMapping['captcha_field'] = ['required', 'string', new \PZone\Core\Rules\CaptchaRule];
        }
    ...
    private function _showLinkRequestForm()
    {
        ...
        pz_check_view($this->templatePath . '.auth.forgot');
        return view(
            $this->templatePath . '.auth.forgot',
            array(
                'title'       => pz_language_render('customer.password_forgot'),
                'layout_page' => 'shop_auth',
                'viewCaptcha' => $viewCaptcha,
                'breadcrumbs' => [
                    ['url'    => '', 'title' => pz_language_render('customer.password_forgot')],
                ],
            )
        );
    }
