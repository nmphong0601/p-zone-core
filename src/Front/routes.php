...
Route::group(
    [
        'middleware' => PZ_FRONT_MIDDLEWARE,
    ],
...
if (config('p-zone.ecommerce_mode', 1)) {
    Route::middleware(PZ_FRONT_MIDDLEWARE)
    ...
    Route::group(
        ...
        function () use ($langUrl, $nameSpaceFrontStore) {
            $prefixShop = pz_config('PREFIX_SHOP') ?? 'shop';
...
Route::group(
    ...
    function () use ($suffix, $nameSpaceFrontContent, $nameSpaceFrontStore) {
        $prefixSearch = pz_config('PREFIX_SEARCH')??'search';
        $prefixContact = pz_config('PREFIX_CONTACT')??'contact';
        $prefixAbout = pz_config('PREFIX_ABOUT')??'about';
        $prefixNews = pz_config('PREFIX_NEWS')??'news';
...
Route::group(
    ...
    function () use ($nameSpaceFrontContent) {
        ...
        Route::get('locale/{code}', function ($code) {
            ...
            if (pz_route('home.lang') === redirect()->back()->getTargetUrl()
            ) {
                return redirect(pz_route('home.lang', ['lang' => $code]));
            }
