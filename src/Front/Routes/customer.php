<?php
$prefixCustomer = pz_config('PREFIX_MEMBER') ?? 'customer';
...
if (file_exists(app_path('Http/Controllers/ShopAccountController.php'))) {
    ...
    $nameSpaceFrontCustomer = 'PZone\Core\Front\Controllers';
}

if (pz_config('customer_verify')) {
...
Route::group(
    ...
    function ($router) use ($suffix, $nameSpaceFrontCustomer) {
        $prefixCustomerOrderList    = pz_config('PREFIX_MEMBER_ORDER_LIST')??'order-list';
        $prefixCustomerOrderDetail  = pz_config('PREFIX_MEMBER_ORDER_DETAIL')??'order-detail';
        $prefixCustomerAddresList   = pz_config('PREFIX_MEMBER_ADDRESS_LIST')??'address-list';
        $prefixCustomerUpdateAddres = pz_config('PREFIX_MEMBER_UPDATE_ADDRESS')??'update-address';
        $prefixCustomerDeleteAddres = pz_config('PREFIX_MEMBER_DELETE_ADDRESS')??'delete-address';
        $prefixCustomerChangePwd    = pz_config('PREFIX_MEMBER_CHANGE_PWD')??'change-password';
        $prefixCustomerChangeInfo   = pz_config('PREFIX_MEMBER_CHANGE_INFO')??'change-infomation';
