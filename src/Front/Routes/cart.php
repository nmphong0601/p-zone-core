...
Route::group(
    ...
    function ($router) use ($suffix, $nameSpaceFrontCart) {
        $prefixCartWishlist = pz_config('PREFIX_CART_WISHLIST') ?? 'wishlist';
        $prefixCartCompare = pz_config('PREFIX_CART_COMPARE') ?? 'compare';
        $prefixCartDefault = pz_config('PREFIX_CART_DEFAULT') ?? 'cart';
