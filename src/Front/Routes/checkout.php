...
if (file_exists(app_path('Http/Controllers/ShopCartController.php'))) {
    ...
    $nameSpaceFrontCart = 'PZone\Core\Front\Controllers';
}
Route::group(
    [
        'prefix' => $langUrl
    ...
    function ($router) use ($suffix, $nameSpaceFrontCart) {
        $prefixCartCheckout = pz_config('PREFIX_CART_CHECKOUT') ?? 'checkout';
        $prefixCartCheckoutConfirm = pz_config('PREFIX_CART_CHECKOUT_CONFIRM') ?? 'checkout-confirm';
        $prefixOrderSuccess = pz_config('PREFIX_ORDER_SUCCESS') ?? 'order-success';
