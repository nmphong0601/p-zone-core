...
class Make extends Command
{
     ...
     * @var string
     */
    protected $signature = 'pz:make {function} {--name=} {--download=0}';
    ...
    .PHP_EOL.'Plugin:   "php artisan pz:make plugin --name=Payment/YourPluginName --download=0"'
    .PHP_EOL.'Template:  "php artisan pz:make tmplate --name=YourTemplateName --download=0"';
    ...
    public function plugin($code = 'Other', $key = '', $download = 0)
    {
        ...
        $pluginKey = pz_word_format_class($key);
        $pluginCode = pz_word_format_class($code);
        if (!in_array($pluginCode, $arrcodePlugin)) {
            $pluginCode = 'Other';
        }
        $pluginUrlKey = pz_word_format_url($key);
        ...
        try {
            File::copyDirectory(base_path('vendor/p-zone/core/src/'.$source), storage_path($tmp));
            File::copyDirectory(base_path('vendor/p-zone/core/src/'.$sourcePublic), storage_path($tmpPublic));
        ...
        try {
            if ($download) {
                ...
                pz_zip(storage_path($this->tmpFolder."/".$sID), $path);
            } else {
