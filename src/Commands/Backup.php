...
class Backup extends Command
{
     ...
     * @var string
     */
    protected $signature = 'pz:backup {--path=} {--includeTables=} {--excludeTables=}';
     ...
     * @var string
     */
    protected $description = 'Backup database "pz:backup --path=abc.sql --includeTables=table1,table2 --excludeTables=t1,t2"';
    const LIMIT = 10;
    ...
    public function handle()
    {
        ...
        if (count(glob(storage_path() . "/backups/*.sql")) >= self::LIMIT) {
            echo json_encode(['error' => 1, 'msg' => pz_language_render('admin.backup.limit_backup')]);
            exit;
        }
        if ($path) {
        ...
        try {
            $databaseName = config('database.connections.'.PZ_CONNECTION.'.database');
            $userName = config('database.connections.'.PZ_CONNECTION.'.username');
            $password = config('database.connections.'.PZ_CONNECTION.'.password');
            $host = config('database.connections.'.PZ_CONNECTION.'.host');
            $pathMysqlBin = config('database.connections.'.PZ_CONNECTION.'.path_mysql_bin');// C:\xampp\mysql\bin
            ...
            pz_report($exception->getMessage());
