    ...
    <div class="card">
      <div class="card-header">
        ...
        <div class="btn-group float-right" style="margin-right: 5px">
            <a href="{{ pz_route_admin('admin_brand.index') }}" class="btn  btn-flat btn-default" title="List"><i class="fa fa-list"></i><span class="hidden-xs"> {{ pz_language_render('admin.back_list') }}</span></a>
        </div>
      @endif
      </div>
      <!-- /.card-header -->
      ...
      <form action="{{ $url_action }}" method="post" accept-charset="UTF-8" class="form-horizontal" id="form-main">
        <div class="card-body">
          ...
          <div class="form-group row {{ $errors->has('name') ? ' text-red' : '' }}">
            <label for="name" class="col-sm-2 col-form-label">{{ pz_language_render('admin.brand.name') }}</label>
            <div class="col-sm-10 ">
          ...
          <div class="form-group row {{ $errors->has('alias') ? ' text-red' : '' }}">
            <label for="alias" class="col-sm-2 col-form-label">{!! pz_language_render('admin.brand.alias') !!}</label>
            <div class="col-sm-10 ">
              <div class="input-group mb-3">
                <div class="input-group-prepend">
                  <span class="input-group-text"><i class="fas fa-pencil-alt"></i></span>
          ...
          <div class="form-group row {{ $errors->has('url') ? ' text-red' : '' }}">
            <label for="url" class="col-sm-2 col-form-label">{!! pz_language_render('admin.brand.url') !!}</label>
            <div class="col-sm-10 ">
              <div class="input-group mb-3">
                <div class="input-group-prepend">
                  <span class="input-group-text"><i class="fas fa-pencil-alt"></i></span>
          ...
          <div class="form-group row {{ $errors->has('image') ? ' text-red' : '' }}">
            <label for="image" class="col-sm-2 col-form-label">{!! pz_language_render('admin.brand.image') !!}</label>
            <div class="col-sm-10 ">
              <div class="input-group mb-3">
                <div class="input-group-prepend">
                  <span class="input-group-text"><i class="fas fa-pencil-alt"></i></span>
                ...
                <div class="input-group-append">
                  <span data-input="image" data-preview="preview_image" data-type="brand"
                      class="btn btn-primary lfm"><i class="fa fa-image"></i>  {{pz_language_render('product.admin.choose_image')}}</span>
                </div>
              </div>
              <div id="preview_image" class="img_holder"><img
                src="{{ pz_file(old('image',$brand['image']??'images/no-image.jpg')) }}">
              </div>
              @if ($errors->has('image'))
              <span class="text-sm">
          ...
          <div class="form-group row {{ $errors->has('sort') ? ' text-red' : '' }}">
            <label for="sort" class="col-sm-2 col-form-label">{!! pz_language_render('admin.brand.sort') !!}</label>
            <div class="col-sm-10 ">
              <div class="input-group mb-3">
                <div class="input-group-prepend">
                  <span class="input-group-text"><i class="fas fa-pencil-alt"></i></span>
...
@if (pz_check_multi_shop_installed())
          ...
          @php
          $listStore = [];
          if (function_exists('pz_get_list_store_of_brand_detail')) {
                  $oldData = pz_get_list_store_of_brand_detail($brand['id'] ?? '');
              } else {
                  $oldData = null;
              }
...
@endif
          ...
          <div class="form-group row {{ $errors->has('status') ? ' text-red' : '' }}">
            <label for="status" class="col-sm-2 col-form-label">{!! pz_language_render('admin.brand.status') !!}</label>
            <div class="col-sm-10 ">
              <div class="input-group mb-3">
        ...
        <div class="card-footer">
          ...
          <button type="submit" class="btn btn-primary float-right">{{ pz_language_render('action.submit') }}</button>
        </div>
        <!-- /.card-footer -->
      </form>
    </div>
        ...
        return new Promise(function(resolve) {
            $.ajax({
                ...
                success: function (data) {
                    if(data.error == 1){
                      alertMsg('error', data.msg, '{{ pz_language_render('action.warning') }}');
                      $.pjax.reload('#pjax-container');
                      return;
                    }else{
                      alertMsg('success', data.msg);
                      window.location.replace('{{ pz_route_admin('admin_brand.index') }}');
                    }
