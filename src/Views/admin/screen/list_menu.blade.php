          ...
          @if ($level0->type ==1)
            <li class="dd-item " data-id="{{ $level0->id }}">
                <div class="dd-handle header-fix  {{ ($level0->id == $id)? 'active-item' : '' }}">
                  {!! pz_language_render($level0->title) !!}
                  <span class="float-right dd-nodrag">
                      <a href="{{ pz_route_admin('admin_menu.edit',['id'=>$level0->id]) }}"><i class="fa fa-edit"></i></a>
                      &nbsp; 
          ...
          @elseif($level0->uri)
            <li class="dd-item" data-id="{{ $level0->id }}">
                <div class="dd-handle {{ ($level0->id == $id)? 'active-item' : '' }}">
                  <i class="{{ $level0->icon }}"></i> {!! pz_language_render($level0->title) !!}
                  <span class="float-right dd-nodrag">
                      <a href="{{ pz_route_admin('admin_menu.edit',['id'=>$level0->id]) }}"><i class="fa fa-edit fa-edit"></i></a>
                      &nbsp; 
                  ...
                  @if($level1->uri)
                    <li class="dd-item" data-id="{{ $level1->id }}">
                        <div class="dd-handle {{ ($level1->id == $id)? 'active-item' : '' }}">
                          <i class="{{ $level1->icon }}"></i> {!! pz_language_render($level1->title) !!}
                          <span class="float-right dd-nodrag">
                              <a href="{{ pz_route_admin('admin_menu.edit',['id'=>$level1->id]) }}"><i class="fa fa-edit fa-edit"></i></a>
                              &nbsp; 
                              ...
                              @if($level2->uri)
                                <li class="dd-item" data-id="{{ $level2->id }}">
                                    <div class="dd-handle {{ ($level2->id == $id)? 'active-item' : '' }}">
                                      <i class="{{ $level2->icon }}"></i> {!! pz_language_render($level2->title) !!}
                                      <span class="float-right dd-nodrag">
                                          <a href="{{ pz_route_admin('admin_menu.edit',['id'=>$level2->id]) }}"><i class="fa fa-edit fa-edit"></i></a>
                                          &nbsp; 
              ...
              <div class="card-header with-border">
                ...
                <div class="card-tools">
                    <div class="btn-group float-right" style="margin-right: 5px">
                        <a href="{{ pz_route_admin('admin_menu.index') }}" class="btn  btn-flat btn-default" title="List"><i class="fa fa-list"></i><span class="hidden-xs"> {{ pz_language_render('admin.back_list') }}</span></a>
                    </div>
                </div>
                @endif
              </div>
                ...
                <form action="{{ $url_action }}" method="post" accept-charset="UTF-8" class="form-horizontal" id="form-main"  enctype="multipart/form-data">
                    ...
                    <div class="card-body">
                      ...
                      <div class="form-group row {{ $errors->has('parent_id') ? ' text-red' : '' }}">
                        <label for="parent_id" class="col-sm-2 col-form-label">{{ pz_language_render('admin.menu.parent') }}</label>
                        <div class="col-sm-10 ">
                      ...
                      <div class="form-group row {{ $errors->has('title') ? ' text-red' : '' }}">
                        <label for="title" class="col-sm-2 col-form-label">{{ pz_language_render('admin.menu.title') }}</label>
                        <div class="col-sm-10 ">
                      ...
                      <div class="form-group row {{ $errors->has('icon') ? ' text-red' : '' }}">
                        <label for="icon" class="col-sm-2 col-form-label">{{ pz_language_render('admin.menu.icon') }}</label>
                        <div class="col-sm-10 ">
                          <div class="input-group mb-3">
                            <div class="input-group-prepend">
                      ...
                      <div class="form-group row {{ $errors->has('uri') ? ' text-red' : '' }}">
                        <label for="uri" class="col-sm-2 col-form-label">{{ pz_language_render('admin.menu.uri') }}</label>
                        <div class="col-sm-10 ">
                          <div class="input-group mb-3">
                            <div class="input-group-prepend">
                              <span class="input-group-text"><i class="fas fa-pencil-alt"></i></span>
                      ...
                      <div class="form-group row {{ $errors->has('sort') ? ' text-red' : '' }}">
                        <label for="sort" class="col-sm-2 col-form-label">{{ pz_language_render('admin.menu.sort') }}</label>
                        <div class="col-sm-10 ">
                          <div class="input-group mb-3">
                            <div class="input-group-prepend">
                              <span class="input-group-text"><i class="fas fa-pencil-alt"></i></span>
...
@push('styles')
<!-- Ediable -->
<link rel="stylesheet" href="{{ pz_file('admin/plugin/nestable/jquery.nestable.min.css')}}">
<link rel="stylesheet" href="{{ pz_file('admin/plugin/iconpicker/fontawesome-iconpicker.min.css')}}">
@endpush

@push('scripts')
<!-- Ediable -->

<script src="{{ pz_file('admin/plugin/nestable/jquery.nestable.min.js')}}"></script>
<script src="{{ pz_file('admin/plugin/iconpicker/fontawesome-iconpicker.min.js')}}"></script>
            ...
            $.ajax({
                ...
                success: function (data) {
                    if(data.error == 1){
                      ...
                      window.location.replace('{{ pz_route_admin('admin_menu.index') }}');
                    }
...
$('.menu-sort-save').click(function () {
    ...
    $.ajax({
      url: '{{ pz_route_admin('admin_menu.update_sort') }}',
      type: 'POST',
      dataType: 'json',
      data: {
        _token: '{{ csrf_token() }}',
