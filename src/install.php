<?php
/**
 * @author NMP <nmphong0601.business@gmail.com>
 */
...
if (request()->method() == 'POST' && request()->ajax()) {
    ...
    switch ($step) {
        ...
        try {
            ...
            $getEnv = str_replace('pz_your_domain', $domain, $getEnv);
            $getEnv = str_replace('pz_database_host', $database_host, $getEnv);
            $getEnv = str_replace('pz_database_port', $database_port, $getEnv);
            $getEnv = str_replace('pz_database_name', $database_name, $getEnv);
            $getEnv = str_replace('pz_database_user', $database_user, $getEnv);
            $getEnv = str_replace('DB_PREFIX=pz_', 'DB_PREFIX='.$database_prefix, $getEnv);
            $getEnv = str_replace('pz_database_password', $database_password, $getEnv);
            $getEnv = str_replace('pz_api_key', $api_key, $getEnv);
            $getEnv = str_replace('pz_admin', $admin_url, $getEnv);
            $getEnv = str_replace('Asia/ho_chi_minh', $timezone_default, $getEnv);

            if (request('only_cms')) {
                $getEnv = str_replace('PZ_ECOMMERCE_MODE=1', 'PZ_ECOMMERCE_MODE=0', $getEnv);
            }
    ...
    echo view('p-zone-front::install', array(
