...
use PZone\Core\Commands\Customize;
use PZone\Core\Commands\Backup;
use PZone\Core\Commands\Restore;
use PZone\Core\Commands\Make;
use PZone\Core\Commands\Infomation;
use PZone\Core\Commands\ClearCart;
use PZone\Core\Commands\Update;
use PZone\Core\Admin\Models\AdminProduct;
use PZone\Core\Front\Middleware\Localization;
use PZone\Core\Front\Middleware\EmailIsVerified;
use PZone\Core\Front\Middleware\Currency;
use PZone\Core\Api\Middleware\ApiConnection;
use PZone\Core\Api\Middleware\ForceJsonResponse;
use PZone\Core\Front\Middleware\CheckDomain;
use PZone\Core\Admin\Middleware\Authenticate;
use PZone\Core\Admin\Middleware\LogOperation;
use PZone\Core\Admin\Middleware\PermissionMiddleware;
use PZone\Core\Admin\Middleware\AdminStoreId;
use PZone\Core\Admin\Middleware\AdminTheme;
use Laravel\Sanctum\Sanctum;
use PZone\Core\Front\Models\PersonalAccessToken;

class PZoneServiceProvider extends ServiceProvider
{
    protected $commands = [
        Customize::class,
        Backup::class,
    ...
    public function boot()
    {
        if(file_exists(base_path('bootstrap/cache/routes-v7.php'))) {
            echo ('<div style="color:red;font-size:10px; background:black;z-index:99999;position:fixed; top:1px;">Sorry!! PZ cannot use route cache. Please delete the file "bootstrap/cache/routes-v7.php" or use the command "php artisan route:clear""</div>');
        }
        Sanctum::usePersonalAccessTokenModel(PersonalAccessToken::class);
        $this->loadTranslationsFrom(__DIR__.'/Lang', 'p-zone');
        ...
        if (!file_exists(public_path('install.php')) && file_exists(base_path('.env'))) {
            ...
            //Check connection
            try {
                DB::connection(PZ_CONNECTION)->getPdo();
            } catch (\Throwable $e) {
    ...
    public function register()
    {
        ...
        $this->app->bind('cart', '\PZone\Core\Library\ShoppingCart\Cart');
        
        $this->mergeConfigFrom(__DIR__.'/Config/admin.php', 'admin');
        $this->mergeConfigFrom(__DIR__.'/Config/validation.php', 'validation');
        $this->mergeConfigFrom(__DIR__.'/Config/lfm.php', 'lfm');
        $this->mergeConfigFrom(__DIR__.'/Config/p-zone.php', 'p-zone');
        $this->mergeConfigFrom(__DIR__.'/Config/cart.php', 'cart');
        $this->mergeConfigFrom(__DIR__.'/Config/middleware.php', 'middleware');
        $this->mergeConfigFrom(__DIR__.'/Config/api.php', 'api');
        $this->loadViewsFrom(__DIR__.'/Views/admin', 'p-zone-admin');
        $this->loadViewsFrom(__DIR__.'/Views/front', 'p-zone-front');
    }

    public function bootPzone()
    {
        // Set store id
        ...
        if (pz_check_multi_shop_installed()) {
            $domain = pz_process_domain_store(url('/'));
        ...
        if (pz_config_global('LOG_SLACK_WEBHOOK_URL')) {
            config(['logging.channels.slack.url' => pz_config_global('LOG_SLACK_WEBHOOK_URL')]);
        }

        //Config language url
        config(['app.seoLang' => (pz_config_global('url_seo_lang') ? '{lang?}/' : '')]);

        //Title app
        config(['app.name' => pz_store('title')]);

        //Config for  email
        if (
            // Default use smtp mode for for supplier if use multi-store
            ($storeId != PZ_ID_ROOT && pz_check_multi_shop_installed())
            ||
            // Use smtp config from admin if root domain have smtp_mode enable
            ($storeId == PZ_ID_ROOT && pz_config_global('smtp_mode'))
            ) {
            $smtpHost     = pz_config('smtp_host');
            $smtpPort     = pz_config('smtp_port');
            $smtpSecurity = pz_config('smtp_security');
            $smtpUser     = pz_config('smtp_user');
            $smtpPassword = pz_config('smtp_password');
            $smtpName     = pz_config('smtp_name');
            $smtpFrom     = pz_config('smtp_from');
            ...
            config(['mail.from.address' => ($smtpFrom ?? pz_store('email'))]);
            config(['mail.from.name' => ($smtpName ?? pz_store('title'))]);
        } else {
            //Set default
            config(['mail.from.address' => (config('mail.from.address')) ? config('mail.from.address'): pz_store('email')]);
            config(['mail.from.name' => (config('mail.from.name')) ? config('mail.from.name'): pz_store('title')]);
        }
        //email

        //Share variable for view
        view()->share('pz_languages', pz_language_all());
        view()->share('pz_currencies', pz_currency_all());
        view()->share('pz_blocksContent', pz_store_block());
        view()->share('pz_layoutsUrl', pz_link());
        view()->share('pz_templatePath', 'templates.' . pz_store('template'));
        view()->share('pz_templateFile', 'templates/' . pz_store('template'));
        //variable model
    ...
    protected function registerPublishing()
    {
        if ($this->app->runningInConsole()) {
            $this->publishes([__DIR__.'/Views/admin'  => resource_path('views/vendor/p-zone-admin')], 'pz:view-admin');
            $this->publishes([__DIR__.'/Views/front'  => resource_path('views/vendor/p-zone-front')], 'pz:view-front');
            $this->publishes([__DIR__.'/Config/admin.php' => config_path('admin.php')], 'pz:config-admin');
            $this->publishes([__DIR__.'/Config/validation.php' => config_path('validation.php')], 'pz:config-validation');
            $this->publishes([__DIR__.'/Config/cart.php' => config_path('cart.php')], 'pz:config-cart');
            $this->publishes([__DIR__.'/Config/api.php' => config_path('api.php')], 'pz:config-api');
            $this->publishes([__DIR__.'/Config/middleware.php' => config_path('middleware.php')], 'pz:config-middleware');
            $this->publishes([__DIR__.'/Config/lfm.php' => config_path('lfm.php')], 'pz:config-lfm');
        }
    }
}
