...
trait DataDefaultSeederTrait
{
    ...
    public $title_en = 'Demo P-Zone : Free Laravel eCommerce';
    public $title_vi = 'Demo P-Zone: Mã nguồn website thương mại điện tử miễn phí';
    ...
    public function runProcess()
    {
        ...
        $db->table(PZ_DB_PREFIX.'admin_menu')->insertOrIgnore($dataMenu);
        
        $dataAdminPermission = $this->dataAdminPermission(PZ_ADMIN_PREFIX);
        $db->table(PZ_DB_PREFIX.'admin_permission')->insertOrIgnore($dataAdminPermission);
        
        $dataAdminRole = $this->dataAdminRole();
        $db->table(PZ_DB_PREFIX.'admin_role')->insertOrIgnore($dataAdminRole);
        
        $dataAdminTemplate = $this->dataAdminTemplate();
        $db->table(PZ_DB_PREFIX.'admin_template')->insertOrIgnore($dataAdminTemplate);

        $dataAdminRolePermission = $this->dataAdminRolePermission();
        $db->table(PZ_DB_PREFIX.'admin_role_permission')->insertOrIgnore($dataAdminRolePermission);

        $dataAdminRoleUser = $this->dataAdminRoleUser();
        $db->table(PZ_DB_PREFIX.'admin_role_user')->insertOrIgnore($dataAdminRoleUser);

        $dataAdminUser = $this->dataAdminUser($this->adminUser, $this->adminPassword, $this->adminEmail);
        $db->table(PZ_DB_PREFIX.'admin_user')->insertOrIgnore($dataAdminUser);

        $dataAdminConfig = $this->dataAdminConfig();
        $db->table(PZ_DB_PREFIX.'admin_config')->insertOrIgnore($dataAdminConfig);

        $dataAdminStore = $this->dataAdminStore($this->adminEmail, $this->language_default, str_replace(['http://','https://', '/install.php'], '', url('/')));
        $db->table(PZ_DB_PREFIX.'admin_store')->insertOrIgnore($dataAdminStore);

        $dataAdminStoreDescription = $this->dataAdminStoreDescription($this->title_en, $this->title_vi);
        $db->table(PZ_DB_PREFIX.'admin_store_description')->insertOrIgnore($dataAdminStoreDescription);

        $dataShopLang = $this->dataShopLang();
        $db->table(PZ_DB_PREFIX.'shop_language')->insertOrIgnore($dataShopLang);

        $dataPaymentStatus = $this->dataPaymentStatus();
        $db->table(PZ_DB_PREFIX.'shop_payment_status')->insertOrIgnore($dataPaymentStatus);

        $dataShippingStatus = $this->dataShippingStatus();
        $db->table(PZ_DB_PREFIX.'shop_shipping_status')->insertOrIgnore($dataShippingStatus);

        $dataShopLayoutPage = $this->dataShopLayoutPage();
        $db->table(PZ_DB_PREFIX.'shop_layout_page')->insertOrIgnore($dataShopLayoutPage);

        $dataShopLayoutPosition = $this->dataShopLayoutPosition();
        $db->table(PZ_DB_PREFIX.'shop_layout_position')->insertOrIgnore($dataShopLayoutPosition);

        $dataShopLink = $this->dataShopLink();
        $db->table(PZ_DB_PREFIX.'shop_link')->insertOrIgnore($dataShopLink);

        $dataShopLinkStore = $this->dataShopLinkStore($dataShopLink);
        $db->table(PZ_DB_PREFIX.'shop_link_store')->insertOrIgnore($dataShopLinkStore);

        $dataShippingStandard = $this->dataShippingStandard();
        $db->table(PZ_DB_PREFIX.'shop_shipping_standard')->insertOrIgnore($dataShippingStandard);

        $dataAtrributeGroup = $this->dataAtrributeGroup();
        $db->table(PZ_DB_PREFIX.'shop_attribute_group')->insertOrIgnore($dataAtrributeGroup);

        $dataCurrency = $this->dataCurrency();
        $db->table(PZ_DB_PREFIX.'shop_currency')->insertOrIgnore($dataCurrency);

        $dataOrderStatus = $this->dataOrderStatus();
        $db->table(PZ_DB_PREFIX.'shop_order_status')->insertOrIgnore($dataOrderStatus);

        $dataPage = $this->dataPage();
        $db->table(PZ_DB_PREFIX.'shop_page')->insertOrIgnore($dataPage);

        $dataPageStore = $this->dataPageStore($dataPage);
        $db->table(PZ_DB_PREFIX.'shop_page_store')->insertOrIgnore($dataPageStore);

        $dataPageDescription = $this->dataPageDescription($dataPage);
        $db->table(PZ_DB_PREFIX.'shop_page_description')->insertOrIgnore($dataPageDescription);

        $dataApiConnection = $this->dataApiConnection();
        $db->table(PZ_DB_PREFIX.'api_connection')->insertOrIgnore($dataApiConnection);

        $dataTax = $this->dataTax();
        $db->table(PZ_DB_PREFIX.'shop_tax')->insertOrIgnore($dataTax);

        $dataWeight = $this->dataWeight();
        $db->table(PZ_DB_PREFIX.'shop_weight')->insertOrIgnore($dataWeight);

        $dataLenght = $this->dataLenght();
        $db->table(PZ_DB_PREFIX.'shop_length')->insertOrIgnore($dataLenght);

        $dataBannerType = $this->dataBannerType();
        $db->table(PZ_DB_PREFIX.'shop_banner_type')->insertOrIgnore($dataBannerType);

        $dataProductProperty = $this->dataProductProperty();
        $db->table(PZ_DB_PREFIX.'shop_product_property')->insertOrIgnore($dataProductProperty);

        $dataLinkGroup = $this->dataLinkGroup();
        $db->table(PZ_DB_PREFIX.'shop_link_group')->insertOrIgnore($dataLinkGroup);
    ...
    public function dataMenu() {
        $dataMenu = [
            ...
            ['id' => 42,'parent_id' => 35,'sort' => 100,'title' => 'p-zone::admin.menu_titles.plugin_other','icon' => 'far fa-circle','uri' => 'admin::plugin/other','key' => null,'type' => 0],
            ['id' => 43,'parent_id' => 35,'sort' => 4,'title' => 'p-zone::admin.menu_titles.plugin_cms','icon' => 'fab fa-modx','uri' => 'admin::plugin/cms','key' => null,'type' => 0],
        ...
        // If use ecommerce
        if (config('p-zone.ecommerce_mode', 1)) {
    ...
    public function dataMenuShop() {
        $dataMenu = [
            ...
            ['id' => 39,'parent_id' => 35,'sort' => 0,'title' => 'p-zone::admin.menu_titles.plugin_payment','icon' => 'far fa-money-bill-alt','uri' => 'admin::plugin/payment','key' => null,'type' => 0],
            ['id' => 40,'parent_id' => 35,'sort' => 1,'title' => 'p-zone::admin.menu_titles.plugin_shipping','icon' => 'fas fa-ambulance','uri' => 'admin::plugin/shipping','key' => null,'type' => 0],
            ['id' => 41,'parent_id' => 35,'sort' => 2,'title' => 'p-zone::admin.menu_titles.plugin_total','icon' => 'fas fa-atom','uri' => 'admin::plugin/total','key' => null,'type' => 0],
            ['id' => 53,'parent_id' => 35,'sort' => 3,'title' => 'p-zone::admin.menu_titles.plugin_fee','icon' => 'fas fa-box','uri' => 'admin::plugin/fee','key' => null,'type' => 0],
    ...
    public function dataAdminTemplate() {
        $dataAdminTemplate = [
            ['id' => '1','key' => 'p-zone-light','name' => 'P-Zone Light','status' => 1]
        ];
    ...
    public function dataAdminStoreDescription($titleE, $titleV) {
        $dataAdminStoreDescription = [
            ['store_id' => PZ_ID_ROOT,'lang' => 'en','title' => $titleE,'description' => 'Free website shopping cart for business','keyword' => '','maintain_content' => '<center><img src="/images/maintenance.png" />
            <h3><span style="color:#e74c3c;"><strong>Sorry! We are currently doing site maintenance!</strong></span></h3>
            </center>','maintain_note' => 'Website is in maintenance mode!'],
            ['store_id' => PZ_ID_ROOT,'lang' => 'vi','title' => $titleV,'description' => 'Laravel shopping cart for business','keyword' => '','maintain_content' => '<center><img src="/images/maintenance.png" />
    ...
    public function dataShopLink() {
        $dataShopLink = [
            ...
            ['id' => 2,'name' => 'P-Zone','url' => '#collection','target' => '_self','module' => null,'group' => 'menu', 'type' => 'collection', 'collection_id' => null, 'status' => '1','sort' => '60'],
            ['id' => 3,'name' => 'About us','url' => 'https://p-zone.nmp-tech.com/en/about.html','target' => '_self','module' => null,'group' => 'menu', 'type' => null, 'collection_id' => 2, 'status' => '1','sort' => '10'],
            ['id' => 4,'name' => 'Github','url' => 'https://github.com/p-zone/p-zone','target' => '_self','module' => null,'group' => 'menu', 'type' => null, 'collection_id' => 2, 'status' => '1','sort' => '20'],
    ...
    public function dataShopLinkStore($dataShopLink) {
        foreach ($dataShopLink as $key => $row) {
            $dataShopLinkStore[] = ['link_id' => $row['id'],'store_id' => PZ_ID_ROOT];
        }
    ...
    public function dataPageStore($dataPage) {
        ...
        foreach ($dataPage as $key => $row) {
            $dataPageStore[] = ['page_id' => $row['id'],'store_id' => PZ_ID_ROOT];
        }
    ...
    public function updateDataVersion() {
        ...
        $db = DB::connection(PZ_CONNECTION);
        $schema = Schema::connection(PZ_CONNECTION);
        //Ony use updated v8.0 -> v8.1
        //--Update menu admin
        $db->table(PZ_DB_PREFIX.'admin_menu')->where('id','27')->update(['parent_id' => 65]);
        $db->table(PZ_DB_PREFIX.'admin_menu')->whereIn('id',['17','18'])->update(['parent_id' => 2]);
        $db->table(PZ_DB_PREFIX.'admin_menu')->whereIn('id',['36','50'])->update(['parent_id' => 70]);
        
        // Add column
        if (!$schema->hasColumn(PZ_DB_PREFIX.'admin_store', 'og_image')) {
            $schema->table(PZ_DB_PREFIX.'admin_store',
        ...
        //Notice
        if (!$schema->hasTable(PZ_DB_PREFIX.'admin_notice')) {
            $schema->create(PZ_DB_PREFIX . 'admin_notice', function (Blueprint $table) {
