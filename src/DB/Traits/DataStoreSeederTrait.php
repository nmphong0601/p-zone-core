...
trait DataStoreSeederTrait
{
    public function getTemplateDefault() {
        return  empty(session('lastStoreTemplate')) ? 'p-zone-light' : session('lastStoreTemplate');
    }
    /**
    ...
    {
        $storeId = empty(session('lastStoreId')) ? PZ_ID_ROOT : session('lastStoreId');

        $db = DB::connection(PZ_CONNECTION);

        $dataConfig = $this->dataConfig($storeId);
        $db->table(PZ_DB_PREFIX.'admin_config')->insert($dataConfig);

        $dataEmailTemplate = $this->dataEmailTemplate($storeId);
        $db->table(PZ_DB_PREFIX.'shop_email_template')->insert($dataEmailTemplate);
        
        $dataShopLink = $this->dataShopLink();
        $db->table(PZ_DB_PREFIX.'shop_link')->insert($dataShopLink);

        $dataShopLinkStore = $this->dataShopLinkStore($dataShopLink, $storeId);
        $db->table(PZ_DB_PREFIX.'shop_link_store')->insert($dataShopLinkStore);


        if (file_exists($fileProcess = resource_path() . '/views/templates/'.$this->getTemplateDefault().'/Provider.php')) {
            include_once $fileProcess;
            if (function_exists('pz_template_install_store')) {
    ...
    public function dataConfig($storeId) {
        $dataConfig = [
            ...
            ['group' => '','code' => 'admin_config','key' => 'ADMIN_NAME','value' => 'P-Zone System','sort' => '0','detail' => 'admin.env.ADMIN_NAME','store_id' => $storeId],
            ['group' => '','code' => 'admin_config','key' => 'ADMIN_TITLE','value' => 'P-Zone Admin','sort' => '0','detail' => 'admin.env.ADMIN_TITLE','store_id' => $storeId],
            ['group' => '','code' => 'admin_config','key' => 'ADMIN_LOGO','value' => 'P-Zone <span class="brand-text font-weight-light">Admin</span>','sort' => '0','detail' => 'admin.env.ADMIN_LOGO','store_id' => $storeId],
            ...
            ['group' => '','code' => 'admin_custom_config','key' => 'facebook_url','value' => 'https://www.facebook.com/','sort' => '0','detail' => 'admin.admin_custom_config.facebook_url','store_id' => $storeId],
            ['group' => '','code' => 'admin_custom_config','key' => 'fanpage_url','value' => 'https://www.facebook.com/','sort' => '0','detail' => 'admin.admin_custom_config.fanpage_url','store_id' => $storeId],
            ['group' => '','code' => 'admin_custom_config','key' => 'twitter_url','value' => 'https://twitter.com/','sort' => '0','detail' => 'admin.admin_custom_config.twitter_url','store_id' => $storeId],
            ...
            ['group' => '','code' => 'admin_custom_config','key' => 'youtube_url','value' => 'https://www.youtube.com/','sort' => '0','detail' => 'admin.admin_custom_config.youtube_url','store_id' => $storeId],
