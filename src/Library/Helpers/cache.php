...
if (!function_exists('pz_clear_cache') && !in_array('pz_clear_cache', config('helper_except', []))) {
    /**
     * Clear cache
     *
     * @param   [string]  $domain
     ...
     */
    function pz_clear_cache($typeCache = 'cache_all', $storeId = null)
    {
        try {
...
if (!function_exists('pz_set_cache') && !in_array('pz_set_cache', config('helper_except', []))) {
    /**
     * [pz_set_cache description]
     *
     ...
     */
    function pz_set_cache($cacheIndex, $value, $time = null)
    {
        if (empty($cacheIndex)) {
            return ;
        }
        $seconds = $time ?? (pz_config_global('cache_time') ?? 600);
