...
use PZone\Core\Library\ShoppingCart\Exceptions\CartAlreadyStoredException;
use PZone\Core\Library\ShoppingCart\Exceptions\UnknownModelException;
use Closure;
...
class Cart
{
     ...
     * @return \PZone\Core\Library\ShoppingCart\Cart
     */
     ...
     * @return \PZone\Core\Library\ShoppingCart\CartItem
     */
    ...
    public function content()
    {
        ...
        foreach ($content as $key => $item) {
            $product = \PZone\Core\Front\Models\ShopProduct::where('id', $item->id)
    ...
    public function total()
    {
        ...
        $total = $content->reduce(function ($total, CartItem $cartItem) {
            return $total + ($cartItem->qty * pz_tax_price($cartItem->price, $cartItem->tax));
        }, 0);
        return $total;
    }
    ...
    */
    public static function getListCart($instance = self::DEFAULT_INSTANCE)
    {
        ...
        $arrCart['subtotal'] = pz_currency_render($cart->subtotal());
        $arrCart['items'] = [];
        if ($cart->count()) {
            foreach ($cart->content() as $key => $item) {
                $product = \PZone\Core\Front\Models\ShopProduct::find($item->id);
                if ($product) {
                    $arrCart['items'][] = [
                        'id'        => $item->id,
                        'rowId'     => $item->rowId,
                        ...
                        'qty'       => $item->qty,
                        'image'     => pz_file($product->getThumb()),
