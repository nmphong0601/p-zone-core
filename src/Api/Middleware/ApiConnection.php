...
namespace PZone\Core\Api\Middleware;
...
class ApiConnection
{
    ...
    {
        if (!pz_config('api_connection_required')) {
            return $next($request);
        }
        ...
        $check = \PZone\Core\Front\Models\ShopApiConnection::check($apiconnection, $apikey);
        if ($check) {
            $check->update(['last_active' => pz_time_now()]);
            return $next($request);
        } else {
